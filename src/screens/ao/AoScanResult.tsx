...
import {
  Center, Button, View, Text, Spinner, 
  ScrollView, Box, useColorModeValue, FlatList
} from 'native-base';
import axios from 'axios';
import * as Location from 'expo-location';
import { 
  ClickableBox,
  DarkModeToggle, ItemCards, ScreenWrapper, unifi_c6, unifi_c7
...
export function AoScanResult({route, navigation}){
  ...
  const handleEventCheckin = (index: number) => {
    ...
    axios.post(
      baseurl + 't/ao/doEventCheckin', theinput, config
    ).then(async (response) => {
      // check for status code
      if(response.data.status_code != '200'){
        alert(JSON.stringify(qrcode));
