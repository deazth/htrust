...
export function AoCLoc({ navigation }) {
  ...
  const [currAddr, setcuraddr] = React.useState('Waiting for location permission');
  ...
  async function translateCurrLoc(lat, long){
    ...
    axios.post(
      baseurl + 't/loc/CoordToAddr', inputcoord, config
    ).then(async (response) => {
  ...
  async function getLastKnownLoc(curlat, curlong){
    ...
    axios.post(
      baseurl + 't/loc/GetCurrentStatus', [], config
    ).then( (response) => {
      // check for status code
      if(response.data.status_code != '200'){
  ...
  async function doCheckIn() {
    ...
    axios.post(
      baseurl + 't/loc/CheckInCoord', inparam, config
    ).then(async (response) => {
      // check for status code
  ...
  async function doCheckOut() {
    ...
    axios.post(
      baseurl + 't/loc/CheckOutCoord', inparam, config
    ).then(async (response) => {
      // check for status code
  ...
  async function doUpdateLoc() {
    ...
    axios.post(
      baseurl + 't/loc/UpdateCoord', inparam, config
    ).then(async (response) => {
      // check for status code
