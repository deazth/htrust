...
export function AoSeatAvail({navigation}) {
  ...
  function searchAvail(){
    ...
    axios.post(
      baseurl + 't/ao/searchAvailableSeat', inputs, config
    ).then((response) => {
      // check for status code
      if(response.data.status_code != '200'){
  ...
  function doSeatReserve(){
    ...
    axios.post(
      baseurl + 't/ao/doSeatReserve', inputs, config
    ).then((response) => {
      // check for status code
      if(response.data.status_code != '200'){
        alert(JSON.stringify(response.data));
        ...
        >
          ...
          >            
            { floorList.length != 0 ? 
                ...
                : bID ? (<Select.Item label="This building has no valid floor" value="" disabled={true} />)
                : (<Select.Item label="Select a building first" value="" disabled={true} />)
            }
          ...
          >
            { fcList.length != 0 ? fcList.map(rec => {
                  return (<Select.Item label={rec.label} value={rec.id + ""} key={rec.id} />);
                }) 
                : fID ? (<Select.Item label="This floor has no valid section" value="" disabled={true} />)
                : (<Select.Item label="Select a floor first" value="" disabled={true} />)
            }
          </Select>
          ...
          <Button key="sm" size="sm" onPress={() => searchAvail()} isLoading={isLoading} isLoadingText="Please wait">Search</Button>
                ...
                <Accordion>
                  { sResult.map(rec => {
                    return (
                      ...
                      <Text color={acco_text_color}>{rec.name} - {rec.count} seats</Text>
                        <Accordion.Icon />
                      </Accordion.Summary>
                      <Accordion.Details>
                        { rec.gotlayout && (
                        ...
                        { rec.fcs.map(fc => {
                          return (
                            ...
                            <Accordion.Summary _expanded={{bg:acco_fc_color}}>
                              <Text color={acco_text_color}>{fc.name} - {fc.count} seats</Text>
                              <Accordion.Icon />
                            </Accordion.Summary>
                            <Accordion.Details>
                            { fc.gotlayout && (
                            ...
                            { fc.seats.map(seat => {
                              return (
                                <Button m={1} p={1}
                                  ...
                                  key={seat.id}
                                  endIcon={<Icon as={FontAwesome5} name="calendar-check" size={4} />}
                ...
                <AlertDialog.Footer>
                  <Button.Group space={2}>
                    <Button
                      ...
                      ref={cancelRef}
                    >Cancel</Button>
                    <Button isLoading={isLoading} isLoadingText="Please Wait" colorScheme="primary" onPress={() => doSeatReserve()}>Reserve</Button>
                  </Button.Group>
                </AlertDialog.Footer>
